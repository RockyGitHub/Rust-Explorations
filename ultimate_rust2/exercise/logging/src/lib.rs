...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog::hop", "A frog hopped, {}", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("Frog will go to sleep because it is out of energy");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog::Default", "Default frog was generated: [{:?}]", frog);
        frog
    }
}
